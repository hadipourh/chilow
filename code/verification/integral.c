 ...
 * 
 * This file is part of the ChiLow Independent Implementation.
 * 
...
static int test_integral_distinguisher(int rounds, const int* active_positions, int num_active,
    ...
    for (int rep = 0; rep < repetitions; rep++) {
        ...
        for (int input = 0; input < total_inputs; input++) {
            ...
            // Compute ChiLow result
            uint64_t result;
            if (use_40bit) {
                result = chilow_reduced_round_40bit(ciphertext_40, base_tweak, 
                                                   base_key_hi, base_key_lo, rounds);
            } else {
                result = chilow_reduced_round_32bit(ciphertext_32, base_tweak, 
                                                   base_key_hi, base_key_lo, rounds);
            }
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 5) {
        ...
        if (argc == 1) {
            ...
            rounds = 3;
            active_positions[0] = 0; active_positions[1] = 1;
            num_active = 2;
            balanced_positions[0] = 0; balanced_positions[1] = 15; 
            balanced_positions[2] = 30; balanced_positions[3] = 31;
            num_balanced = 4;
            repetitions = 10;
